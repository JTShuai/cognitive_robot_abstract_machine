...
_ENTITY_TYPES = {
    OWL.Class: OntologyEntityKind.CLASS,
    OWL.ObjectProperty: OntologyEntityKind.OBJECT_PROPERTY,
    OWL.DatatypeProperty: OntologyEntityKind.DATA_PROPERTY,
    OWL.AnnotationProperty: OntologyEntityKind.ANNOTATION_PROPERTY,
    OWL.NamedIndividual: OntologyEntityKind.NAMED_INDIVIDUAL,
}
...
def _merge_rdf_xml(
    ...
    for entity_type, kind in _ENTITY_TYPES.items():
        for subject in graph.subjects(RDF.type, entity_type):
            if not isinstance(subject, URIRef):
                continue
            entry = accumulators.setdefault(str(subject), _EntityAccumulator())
            entry.kinds.add(kind)
            entry.source_ids.add(source_id)
            entry.labels.update(_texts(graph.objects(subject, RDFS.label)))
            entry.comments.update(_texts(graph.objects(subject, RDFS.comment)))
            for parent in (
                *graph.objects(subject, RDFS.subClassOf),
                *graph.objects(subject, RDFS.subPropertyOf),
            ):
                if isinstance(parent, URIRef):
                    entry.parent_iris.add(str(parent))


def _texts(values: Iterable[object]) -> set[str]:
    """
    Whitespace-normalized, non-empty literal values.
    """
    texts = (" ".join(str(value).split()) for value in values)
