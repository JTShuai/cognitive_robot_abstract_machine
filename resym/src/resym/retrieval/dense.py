...
:class:`~resym.retrieval.index.RetrievalConfig` by
fragment id (``dense_scorer_by_id``), so retrieval stays deterministic
and retrieval never performs the heavy encoding step online.
...
CLI (one-time per release, run on a machine with sentence-transformers)::
    ...
    uv run python -m resym.retrieval.dense augment_dataset/corpus_release/r1 \
