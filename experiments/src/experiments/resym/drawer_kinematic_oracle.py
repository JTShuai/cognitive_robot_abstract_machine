...
class DrawerOracleUnsupportedError(Exception):
    """
    The controlled drawer oracle cannot realize the requested transition.
    """


DrawerRealizationFunction = Callable[
    [EvaluationContext, ObjectUniverse, ExecutionRequest], None
]

DRAWER_REALIZATIONS: dict[str, DrawerRealizationFunction] = {}


def drawer_realization(
    capability_uid: str,
) -> Callable[[DrawerRealizationFunction], DrawerRealizationFunction]:
    def decorate(
        function: DrawerRealizationFunction,
    ) -> DrawerRealizationFunction:
        DRAWER_REALIZATIONS[capability_uid] = function
        return function

    return decorate


@drawer_realization(INTERACTION_NAVIGATION_CAPABILITY_UID)
def reach_articulation_interaction(
    context: EvaluationContext, universe: ObjectUniverse, request: ExecutionRequest
) -> None:
    if context.robot.drive is None:
        raise DrawerOracleUnsupportedError(
            f"robot '{type(context.robot).__name__}' has no mobile base"
        )
    ...
    context.robot.drive.origin = context.witness_base_poses[key]
...
@drawer_realization(ARTICULATION_CAPABILITY_UID)
...
@dataclass
class DrawerExperimentRealization(PlatformSkillRealization):
    """
    ...
    def available_capabilities(self, robot: AbstractRobot) -> frozenset[str]:
        return frozenset(DRAWER_REALIZATIONS)
    ...
    def execute(
        ...
        if uid not in DRAWER_REALIZATIONS:
            raise UnknownCapabilityError(uid)
        DRAWER_REALIZATIONS[uid](context, universe, request)
        return PlatformExecutionResult.succeeded("KINEMATIC_EXECUTION_SUCCEEDED")


# %% capability feasibility

DrawerFeasibilityProcedure = Callable[
    ["DrawerExperimentFeasibility", EvaluationContext, tuple[GroundedObject, ...]], bool
]

DRAWER_FEASIBILITY: dict[str, DrawerFeasibilityProcedure] = {}


def drawer_feasibility(
    capability_uid: str,
) -> Callable[[DrawerFeasibilityProcedure], DrawerFeasibilityProcedure]:
    """
    Register the procedure deciding one capability's kinematic feasibility.
    """

    def decorate(
        function: DrawerFeasibilityProcedure,
    ) -> DrawerFeasibilityProcedure:
        DRAWER_FEASIBILITY[capability_uid] = function
        return function

    return decorate


@drawer_feasibility(ARTICULATION_CAPABILITY_UID)
def interaction_reachable_from_current_base(
    feasibility: DrawerExperimentFeasibility,
    context: EvaluationContext,
    ...
    if not has_interaction_point(patient):
        return False
    return _arm_reaches(
...
@drawer_feasibility(INTERACTION_NAVIGATION_CAPABILITY_UID)
def interaction_reachable_from_sampled_base(
    ...
    if context.robot.drive is None:
        raise GroundingFailure(
            ...
            f"robot '{type(context.robot).__name__}' has no drive: base "
...
@dataclass
class DrawerExperimentFeasibility(CapabilityFeasibility):
    """
    ...
    def feasible(
        ...
        if capability_uid not in DRAWER_FEASIBILITY:
            raise GroundingFailure(
                GroundingFailureCode.UNSUPPORTED_QUERY,
                f"no kinematic feasibility procedure for capability "
                f"'{capability_uid}'",
            )
        return DRAWER_FEASIBILITY[capability_uid](self, context, arguments)
...
def _reaching_configuration_collides(
    ...
    with world.reset_state_context():
        if context.robot.drive is not None:
            # a fixed base already stands at world_T_base
            context.robot.drive.origin = world_T_base
