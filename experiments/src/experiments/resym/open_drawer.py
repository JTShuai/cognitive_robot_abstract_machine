...
from experiments.resym.seed_library import LIBRARY_PATH, save_seed_library
...
def run_scene(
    ...
    opened = joint_fraction(connection) >= context.budgets.open_fraction
    print(
...
def main() -> None:
    ...
    if not LIBRARY_PATH.exists():
        save_seed_library()
    library = SymbolLibrary.load(LIBRARY_PATH)
    recorder.record_library(library)
    print(
        f"library: {LIBRARY_PATH.name} "
